...
assert_Nanopublication(Label, AgentURI, Method, _NanopubURI) :-
    	...
    	% define URI for the nanopublication: JESUS: now its same as head
        % concat_atom([Label, '_nano'], NanopubID), 
        % rdf_global_id(data:NanopubID, NanopubURI),
		rdf_assert(HeadURI, rdf:type, nanopub:'Nanopublication', HeadURI), %before:NanopubURI
    	% define URI for the assertion graph. JESUS, now its just the label
        % concat_atom([Label, '_assertion'], AssertionID),
        % rdf_global_id(data:AssertionID, AssertionURI),
        rdf_global_id(data:Label, AssertionURI),
		% rdf_assert(NanopubURI, nanopub:'hasAssertion', AssertionURI, HeadURI),
        rdf_assert(HeadURI, nanopub:'hasAssertion', AssertionURI, HeadURI),
    	% define URI for the provenance graph
        concat_atom([Label, '_provenance'], ProvID),
        rdf_global_id(data:ProvID, ProvURI),
		rdf_assert(HeadURI, nanopub:'hasProvenance', ProvURI, HeadURI),
		...
		rdf_assert(HeadURI, rdf:type, nanopub:'Nanopublication', HeadURI),
		rdf_assert(HeadURI, nanopub:'hasPublicationInfo', PubInfoURI, HeadURI),
		rdf_assert(HeadURI, prov:'generatedAtTime', PubInfoURI, PubInfoURI), %% Function for timestamp!!!
		rdf_assert(HeadURI, prov:'wasAttributedTo', AgentURI, PubInfoURI),
		rdf_assert(HeadURI, prov:'wasGeneratedBy', Method, PubInfoURI).
...
deleteNanopublication(NanopubURI) :-
    rdf(NanopubURI, nanopub:'hasProvenance', ProvURI, NanopubURI),
    rdf(NanopubURI, nanopub:'hasAssertion', AssertionURI, NanopubURI),
	rdf(NanopubURI, nanopub:'hasPublicationInfo', PubInfoURI, NanopubURI),
    rdf_retractall(_, _, _, PubInfoURI),
    rdf_retractall(_, _, _, ProvURI),
    rdf_retractall(_, _, _, AssertionURI),
    rdf_retractall(NanopubURI, nanopub:'hasProvenance', ProvURI, NanopubURI),
    rdf_retractall(NanopubURI, nanopub:'hasAssertion', AssertionURI, NanopubURI),
	rdf_retractall(NanopubURI, nanopub:'hasPublicationInfo', PubInfoURI, NanopubURI).
